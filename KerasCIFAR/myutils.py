...
def compute_mle(dists):
    ...
    lid = -1.0*k/(log_sum)
    return lid
    #f = lambda v: - k / np.sum(np.log(v / v[-1]))s

# def compute_mle(dists):
#     dists = dists[dists != 0] #eliminate any 0 to avoid log error
#     k = len(dists)
#     log_vals = np.log( dists/dists[-1] )
...
def get_crosslid_score(real_outputs, fake_outputs):
    a = real_outputs
    b = fake_outputs
    a2 = np.square(a)
    b2 = np.square(b)
    row_norms_a = np.sum(a2, axis=1)
    row_norms_b = np.sum(b2, axis=1)
    row_norms_a = np.reshape(row_norms_a, [-1, 1])
    row_norms_b = np.reshape(row_norms_b, [1, -1])
    sq_dists = (row_norms_a - 2 * np.dot(a, np.transpose(b))) + row_norms_b
    dists = np.sqrt(sq_dists + 1e-6) #sqrt in loss function?
    dists = clip_log_outputs(dists)
    max_dists = np.max(dists, axis=1)
    log_dists = np.log(dists)
    max_log_dists = np.log(max_dists)
    avg_log_dists = np.mean(log_dists, axis=1)
    max_minus_avg_dists = max_log_dists - avg_log_dists
    clids = 1.0/max_minus_avg_dists
    return np.reshape(clids, [-1, 1])
...
def scale_value(X, to_min_max, verbose = False):
    from_min_max = get_min_max(X)
    if (verbose):   print('from: ', from_min_max)
    #Conver to [0, 1]
...
def show_cifar10_images(image_array):
    size = image_array.shape[0]
    rows = int(math.sqrt(size))
    cols = int(size*1.0/rows)
    plt.figure(figsize=(6, 6))
